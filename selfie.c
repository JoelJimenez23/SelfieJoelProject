...
void emit_ts();
void implement_ts(uint64_t * context);

void emit_cas();
void implement_cas(uint64_t *context);
...
void emit_get_context();
void implement_get_context(uint64_t *context);
...
uint64_t SYSCALL_FORK = 69;
uint64_t SYSCALL_WAIT = 78;
uint64_t SYSCALL_THREAD = 23;
uint64_t SYSCALL_TS = 24;
uint64_t SYSCALL_CAS = 25;
uint64_t SYSCALL_THREAD_JOIN = 26;
...
uint64_t SYSCALL_GET_CONTEXT = 43;
...
// +----+-----------------+
// | 32 | pid 					  | process identifier
// | 33 | chldrn-list head| children list head
// | 34 | n_children      | number of children
// | 35 | child exit_code | child exception
// | 36 | next_sibling    | pointer to next sibling context
// | 37 | prev_sibling    | pointer to prev sibling context
// | 38 | state 					| state of the context (Ready,Block)
// +----+-----------------+
// | 39 | thread_id 			| number of thread
// | 40 | next_thread 	  | pointer to next_thread
// | 41 | prev_thread			| pointer to prev_thread
// | 42 | joined 					| is it joined to another thread?
...
uint64_t children_list_head(uint64_t *context) {return (uint64_t)(context+33);}
uint64_t n_children(uint64_t *context) {return (uint64_t)(context+34);}
uint64_t child_exit_code(uint64_t *context){return (uint64_t )(context+35);}
uint64_t next_sibling(uint64_t *context) { return (uint64_t)(context+36); }
uint64_t prev_sibling(uint64_t *context) { return (uint64_t)(context+37); }
uint64_t state(uint64_t *context) { return (uint64_t)(context+38); }
uint64_t thread_id(uint64_t *context) {return (uint64_t)(context+39);}
uint64_t next_thread(uint64_t *context) { return (uint64_t)(context+40); }
uint64_t prev_thread(uint64_t *context) { return (uint64_t)(context+41); }
uint64_t joined(uint64_t *context) { return (uint64_t)(context+42); }
...
uint64_t get_pid(uint64_t *context){ return (uint64_t)*(context+32); }
uint64_t *get_children_list_head(uint64_t *context){ return (uint64_t*)*(context+33); }
uint64_t get_n_children(uint64_t *context){ return (uint64_t)*(context+34); }
uint64_t get_child_exit_code(uint64_t *context){ return (uint64_t )*(context+35); }
uint64_t *get_next_sibling(uint64_t *context) { return (uint64_t *)*(context+36); }
uint64_t *get_prev_sibling(uint64_t *context) { return (uint64_t *)*(context+37); }
uint64_t get_state(uint64_t *context) { return (uint64_t)*(context+38); }
uint64_t get_thread_id(uint64_t *context) {return (uint64_t)*(context+39); }
uint64_t *get_next_thread(uint64_t *context) { return (uint64_t *)*(context+40); }
uint64_t *get_prev_thread(uint64_t *context) { return (uint64_t *)*(context+41); }
uint64_t get_joined(uint64_t* context) { return (uint64_t)*(context+42); }
...
void set_children_list_head(uint64_t *context,uint64_t *children_list_head){ *(context+33) = (uint64_t)children_list_head; }
void set_n_children(uint64_t *context,uint64_t n_children){ *(context+34) = n_children; }
void set_child_exit_code(uint64_t *context,uint64_t child_exception){ *(context+35) = child_exception; }
void set_next_sibling(uint64_t *context, uint64_t *next_sibling) { *(context + 36) = (uint64_t)next_sibling; }
void set_prev_sibling(uint64_t *context, uint64_t *prev_sibling) { *(context + 37) = (uint64_t)prev_sibling; }
void set_state(uint64_t *context,uint64_t state) { *(context+38) = state; }
void set_thread_id(uint64_t *context,uint64_t thead_id){ *(context+39) = thead_id; }
void set_next_thread(uint64_t *context, uint64_t *next_thread) { *(context + 40) = (uint64_t)next_thread; }
void set_prev_thread(uint64_t *context, uint64_t *prev_thread) { *(context + 41) = (uint64_t)prev_thread; }
void set_joined(uint64_t *context, uint64_t join) { *(context+42) = (uint64_t)join;}
...
void print_context(uint64_t *context) {
	if (debug_context) {
		...
		printf("======= CONTEXT INFO =======\n");
		printf("next_context        : addr = %p, value = %p\n", 
				 (void *)next_context(context), (void *)get_next_context(context));
		printf("prev_context        : addr = %p, value = %p\n", 
				 (void *)prev_context(context), (void *)get_prev_context(context));
		printf("program_counter     : addr = %p, value = 0x%lX\n", 
				 (void *)program_counter(context), get_pc(context));
		uint64_t *regs_ptr = get_regs(context);
		printf("registers           : addr = %p\n", (void *)regs(context));
		printf("  ├── zero : 0x%016lX\n", regs_ptr[REG_ZR]);
		printf("  ├── ra   : 0x%016lX\n", regs_ptr[REG_RA]);
		printf("  ├── a0   : 0x%016lX\n", regs_ptr[REG_A0]);
		printf("  └── a7   : 0x%016lX\n", regs_ptr[REG_A7]);
		printf("  └── sp   : 0x%016lX\n", regs_ptr[REG_SP]);
		printf("page_table          : addr = %p, value = %p\n", 
				 (void *)page_table(context), (void *)get_pt(context));
		printf("lowest_lo_page      : addr = %p, value = 0x%lX\n", 
				 (void *)lowest_lo_page(context), get_lowest_lo_page(context));
		printf("highest_lo_page     : addr = %p, value = 0x%lX\n", 
				 (void *)highest_lo_page(context), get_highest_lo_page(context));
		printf("lowest_hi_page      : addr = %p, value = 0x%lX\n", 
				 (void *)lowest_hi_page(context), get_lowest_hi_page(context));
		printf("highest_hi_page     : addr = %p, value = 0x%lX\n", 
				 (void *)highest_hi_page(context), get_highest_hi_page(context));
		printf("code_start          : addr = %p, value = 0x%lX\n", 
				 (void *)code_seg_start(context), get_code_seg_start(context));
		printf("code_size           : addr = %p, value = 0x%lx\n", 
				 (void *)code_seg_size(context), get_code_seg_size(context));
		printf("data_start          : addr = %p, value = 0x%lX\n", 
				 (void *)data_seg_start(context), get_data_seg_start(context));
		printf("data_size           : addr = %p, value = %lu\n", 
				 (void *)data_seg_size(context), get_data_seg_size(context));
		printf("heap_start          : addr = %p, value = 0x%lX\n", 
				 (void *)heap_seg_start(context), get_heap_seg_start(context));
		printf("program_break       : addr = %p, value = 0x%lX\n", 
				 (void *)program_break(context), get_program_break(context));
		printf("exception           : addr = %p, value = %lu\n", 
				 (void *)exception(context), get_exception(context));
		printf("faulting_page       : addr = %p, value = 0x%lX\n", 
				 (void *)fault(context), get_fault(context));
		printf("exit_code           : addr = %p, value = %lu\n", 
				 (void *)exit_code(context), get_exit_code(context));
		printf("parent              : addr = %p, value = %p\n", 
				 (void *)parent(context), (void *)get_parent(context));
		printf("virtual_context     : addr = %p, value = %p\n", 
				 (void *)virtual_context(context), (void *)get_virtual_context(context));
		printf("name                : addr = %p, value = %s\n", 
				 (void *)name(context), get_name(context));
		printf("instr_counter       : addr = %p, value = %lu\n", 
				 (void *)(context + 21), get_ic_all(context));
		printf("malloc_counter      : addr = %p, value = %lu\n", 
				 (void *)lc_malloc(context), get_lc_malloc(context));
		printf("syscall_counter     : addr = %p, value = %lu\n", 
				 (void *)ec_syscall(context), get_ec_syscall(context));
		printf("page_fault_counter  : addr = %p, value = %lu\n", 
				 (void *)ec_page_fault(context), get_ec_page_fault(context));
		printf("timer_interrupts    : addr = %p, value = %lu\n", 
				 (void *)ec_timer(context), get_ec_timer(context));
		printf("peak_stack_size     : addr = %p, value = %lu\n", 
				 (void *)(context + 26), get_mc_stack_peak(context));
		printf("mapped_heap         : addr = %p, value = %lu\n", 
				 (void *)mc_mapped_heap(context), get_mc_mapped_heap(context));
		printf("used_list_head      : addr = %p, value = ", 
				 (void *)used_list_head(context));iterate_used_contexts();
		printf("free_list_head      : addr = %p, value = ", 
				 (void *)free_list_head(context));iterate_free_contexts();
		printf("gcs_counter         : addr = %p, value = %lu\n", 
				 (void *)(context + 30), get_gcs_in_period(context));
		printf("gc_enabled          : addr = %p, value = %lu\n", 
				 (void *)(context + 31), get_use_gc_kernel(context));
		printf("pid                 : addr = %p, value = %lu\n", 
				 (void *)(context + 32), get_pid(context));
		printf("children_list_head  : addr = %p, value = ", 
				 (void *)(context + 33));iterate_children_contexs(context);
		printf("n_children          : addr = %p, value = %ld\n", 
				 (void *)(context + 34), get_n_children(context));
		printf("child exception     : addr = %p, value = %ld\n", 
				 (void *)(context + 35), get_child_exit_code(context));
		printf("next_sibling        : addr = %p, value = %p\n", 
				 (void *)(context + 36), (void *)get_next_sibling(context));
		printf("prev_sibling        : addr = %p, value = %p\n", 
				 (void *)(context + 37), (void *)get_prev_sibling(context));
		printf("state 		    : addr = %p, value = %ld\n", 
				 (void *)(context + 38), get_state(context));
		printf("thread_id    	    : addr = %p, value = %ld\n",
				 (void*)(context+39),get_thread_id(context));
		printf("next_thread        : addr = %p, value = %p\n", 
				 (void *)(context + 40), (void *)get_next_thread(context));
		printf("prev_thread        : addr = %p, value = %p\n", 
				 (void *)(context + 41), (void *)get_prev_thread(context));
		printf("joined 						 : addr = %p, value = %ld\n",
				 (void *)(context + 42), get_joined(context));
		printf("main_thread        : addr = %p, value = %p\n",
				 (void *)(context + 43), (void *)get_main_thread(context));
		printf("============================\n");
	}
}
...
void delete_from_list(uint64_t* context,uint64_t** list){
	//caso 1 ,  prev <-> context <-> NULL 
	if(get_prev_context(context) != (uint64_t*)0 && get_next_context(context) == (uint64_t*)0)
		set_next_context(get_prev_context(context),(uint64_t*)0);
		//caso 2 ,  NULL <-> context <-> next
	else if(get_prev_context(context) == (uint64_t*)0 && get_next_context(context) != (uint64_t*)0){
		set_prev_context(get_next_context(context),(uint64_t*)0);
		*list = get_next_context(context);
	}	
	//caso 3 ,  prev <-> context <-> next
	else if(get_prev_context(context) != (uint64_t*)0 && get_next_context(context) != (uint64_t*)0){
...
void use_context(uint64_t * context){
	if(get_state(context) == WAIT)
		delete_from_list(context,&waiting_lock_contexts);
	else if (get_state(context) == BLOCKED)
		delete_from_list(context,&blocked_contexts);
	add_to_list(context,&used_contexts);
}

void wait_context(uint64_t * context){
...
void delete_thread(uint64_t* context){
	//caso 1 ,  prev <-> context <-> NULL 
	if(get_prev_thread(context) != (uint64_t*)0 && get_next_thread(context) == (uint64_t*)0)
		set_next_thread(get_prev_thread(context),(uint64_t*)0);
		//caso 2 ,  NULL <-> context <-> next
	else if(get_prev_thread(context) == (uint64_t*)0 && get_next_thread(context) != (uint64_t*)0){
		set_prev_thread(get_next_thread(context),(uint64_t*)0);
		set_next_thread(get_main_thread(context),get_next_thread(context));
	}	
	//caso 3 ,  prev <-> context <-> next
	else if(get_prev_thread(context) != (uint64_t*)0 && get_next_thread(context) != (uint64_t*)0){
		set_next_thread(get_prev_thread(context),get_next_thread(context));
		set_prev_thread(get_next_thread(context),get_prev_thread(context));
	}
	else if (get_prev_thread(context) == (uint64_t *)0 && get_next_thread(context) == (uint64_t *)0) {
		set_next_thread(get_main_thread(context),(uint64_t *)0);
	}
}
...
void iterate_free_contexts() {
	uint64_t *current = free_contexts;
...
void iterate_used_contexts() {
	uint64_t *current = used_contexts;

	printf("Used Contexts:\n |->");
	while (current != (uint64_t *)0) {
		printf("  [%p] <--> ", current);
...
void iterate_blocked_contexts() {
	uint64_t *current = blocked_contexts;

	printf("Blocked Contexts (Forward Traversal):\n |->");
...
void iterate_children_contexs(uint64_t * context){
	uint64_t *current = get_children_list_head(context);

	printf("|->");

	while (current != (uint64_t *)0) {
		printf("  [%p] <--> ", current);
		current = (uint64_t *)current[36]; // next is at offset 0
	}

	printf("NULL\n");
}

void iterate_thread_contexs(uint64_t * context){
	uint64_t *current = context;

	printf("|->");

	while (current != (uint64_t *)0) {
		printf("  [%p] <--> ", current);
		current = (uint64_t *)current[40]; // next is at offset 0
	}
...
void iterate_waiting_contexs(){
	...
	printf("Waiting Contexts (Forward Traversal):\n |->");
...
void implement_exit(uint64_t *context)
{
	//printf("context %p entered to exit \n",context);
  // parameter
  ...
  if (debug_syscalls)
  {
	...
	*(get_regs(context)+REG_A0) = get_pid(context);
...
void implement_brk(uint64_t *context)
{
  ...
  // attempt to update program break
	...
	if(get_next_thread(context) != (uint64_t*)0){  
		//printf("tiempo indefinido\n");
		print_context(get_next_thread(context));
	}
...
void implement_fork(uint64_t* context){
	uint64_t * child = create_context(MY_CONTEXT,0);
	fork_context(context,child);
	*(get_regs(context)+REG_A0) = get_pid(child);
	set_pc(context,get_pc(context)+INSTRUCTIONSIZE);

	*(get_regs(child)+REG_A0) = (uint64_t)0;
	set_pc(child,get_pc(child)+INSTRUCTIONSIZE);
	
	if(get_children_list_head(context) == (uint64_t *)0){  
		set_children_list_head(context,child);
	}
	else{
		set_next_sibling(get_children_list_head(context),child);
		set_prev_sibling(child,get_children_list_head(context));
	}

	set_n_children(context,get_n_children(context)+1);
	set_parent(child,context);
	set_child_exit_code(context,get_exit_code(child));

}

void emit_wait(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("wait"),0,PROCEDURE,UINT64_T,0,code_size);
	emit_addi(REG_A0,REG_ZR,0);
...
void implement_wait(uint64_t * context){
	//mandar a dormir a blocked_contexts
	if(get_n_children(context) == 0){
...
void emit_thread(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("thread"),0,PROCEDURE,UINT64_T,0,code_size);
	emit_addi(REG_A0,REG_ZR,0);
...
void implement_thread(uint64_t * context){
	if(get_thread_id(context) == 0){
		uint64_t *thread = create_context(MY_CONTEXT,0);	
		thread_context(context,thread);
		
		if(get_next_thread(context) == (uint64_t*)0){
			set_next_thread(context,thread);
		}else {
			set_next_thread(thread,get_next_thread(context));
			set_prev_thread(get_next_thread(context),thread);
			set_next_thread(context,thread);
			set_prev_thread(thread,context);
		}
...
void emit_ts(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("test_and_set"),0,PROCEDURE,UINT64_T,0,code_size);
	emit_addi(REG_A0,REG_ZR,0);
	emit_addi(REG_A7,REG_ZR,SYSCALL_TS);
	emit_ecall();
	emit_jalr(REG_ZR,REG_RA,0);
}

void implement_ts(uint64_t * context){
	uint64_t* old_value = (uint64_t *)*(get_regs(context)+REG_A0);
	*(get_regs(context)+REG_A0) = 1;
	*(get_regs(context)+REG_A1) = *old_value;
	set_pc(context,get_pc(context)+INSTRUCTIONSIZE);
}

void emit_cas(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("compare_and_swap"),0,PROCEDURE,UINT64_T,3,code_size);
	emit_addi(REG_A0,REG_ZR,0);
	emit_addi(REG_A1,REG_ZR,0);
	emit_addi(REG_A2,REG_ZR,0);
	emit_addi(REG_A7,REG_ZR,SYSCALL_CAS);
	emit_ecall();
	emit_jalr(REG_ZR,REG_RA,0);
}

void implement_cas(uint64_t * context){
	uint64_t* addr = (uint64_t *)*(get_regs(context)+REG_A0);
	uint64_t old_value = *(get_regs(context)+REG_A1);
	uint64_t new_value = *(get_regs(context)+REG_A2);

	if(*addr == old_value){
		*addr = new_value;
		*(get_regs(context)+REG_A0) = 1;
	} else { *(get_regs(context)+REG_A0) = 0; }
	set_pc(context,get_pc(context)+INSTRUCTIONSIZE);
}


void emit_thread_join(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("thread_join"),0,PROCEDURE,UINT64_T,0,code_size);
	emit_addi(REG_A0,REG_ZR,0);
...
void implement_thread_join(uint64_t *context){
	if(get_main_thread(context) != (uint64_t*)0)	
		set_joined(get_main_thread(context),1);
	else
		set_joined(context,1);
...
void emit_getpid(){
	...
	emit_addi(REG_A0,REG_ZR,0);
	emit_addi(REG_A7,REG_ZR,SYSCALL_GETPID);
	emit_ecall();
	emit_jalr(REG_ZR,REG_RA,0);
}
...
void emit_get_context(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("get_context"),0,PROCEDURE,UINT64_T,0,code_size);
	emit_addi(REG_A7,REG_ZR,SYSCALL_GET_CONTEXT);
	emit_ecall();
	emit_jalr(REG_ZR,REG_RA,0);
}

void implement_get_context(uint64_t *context){
	*(get_regs(context)+REG_A0) = (uint64_t)context;
	set_pc(context,get_pc(context)+INSTRUCTIONSIZE);
}

void emit_sleep(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("sleep"),0,PROCEDURE,UINT64_T,0,code_size);
...
void emit_awake(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("awake"),0,PROCEDURE,UINT64_T,1,code_size);
	emit_load(REG_A0,REG_SP,0);
	emit_addi(REG_SP,REG_SP,WORDSIZE);
	emit_addi(REG_A7,REG_ZR,SYSCALL_AWAKE);
	emit_ecall();
	emit_jalr(REG_ZR,REG_RA,0);
}
...
void emit_printf(){
	create_symbol_table_entry(GLOBAL_TABLE,string_copy("printf"),0,PROCEDURE,UINT64_T,2,code_size);
	emit_load(REG_A0,REG_SP,0);
	emit_addi(REG_SP,REG_SP,WORDSIZE);
	emit_load(REG_A1,REG_SP,0);
	emit_addi(REG_SP,REG_SP,WORDSIZE);
	emit_addi(REG_A7,REG_ZR,SYSCALL_PRINTF);
	emit_ecall();
	emit_jalr(REG_ZR,REG_RA,0);
}

void implement_printf(uint64_t *context){
	uint64_t key = *(get_regs(context)+REG_A0);
	if(key == 0){
		printf("%ld\n",(uint64_t)*(get_regs(context)+REG_A1));
	} 
	else if (key == 1){
		printf("%p\n",(uint64_t*)*(get_regs(context)+REG_A1));
	}
...
uint64_t do_load()
{
  ...
  if (is_virtual_address_valid(vaddr, WORDSIZE))
  {
		//printf("[do_load] pc = 0x%lX, rs1 = %ld, imm = %ld, base = 0x%lX, vaddr = 0x%lX\n", pc, rs1, imm, *(registers + rs1), vaddr);
...
void do_ecall()
{
  ...
  else
  {
    read_register(REG_A0);
		...
		else if (*(registers+REG_A7) == SYSCALL_THREAD){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_THREAD_JOIN){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_TS){
			write_register(REG_A0);
			write_register(REG_A1);
		}
		else if (*(registers+REG_A7) == SYSCALL_CAS){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_CREATE_LOCK){
			write_register(REG_A0);
		}
		...
		else if (*(registers+REG_A7) == SYSCALL_GETPID){
				write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_EXIT){
				write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_CREATE_SEMAPHORE){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_SEMAPHORE_WAIT){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_SEMAPHORE_POS){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_GETTHREADID){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_CRITICAL_SECTION_COUNTER){
			write_register(REG_A0);
		}
		...
		else if (*(registers+REG_A7) == SYSCALL_REQUEST_BANKERS){
			write_register(REG_A0);
		}
		...
		else if (*(registers+REG_A7) == SYSCALL_CREATE_ARRAY){
			write_register(REG_A0);
		}
		...
		else if (*(registers+REG_A7) == SYSCALL_GET_CONTEXT){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_SLEEP){
			write_register(REG_A0);	
		}
		else if (*(registers+REG_A7) == SYSCALL_AWAKE){
			write_register(REG_A0);
		}
		else if (*(registers+REG_A7) == SYSCALL_PRINTF){
			read_register(REG_A1);
			write_register(REG_A0);	
		}
		else if (*(registers + REG_A7) != SYSCALL_EXIT)
...
void fetch()
{
  ...
  {
		/*
		printf("context %p ",current_context);
		printf("PC 0x%lX ",pc);
		uint64_t stack_page;
		printf("stack_value 0x%lX ",*(get_regs(current_context)+REG_SP));
		stack_page = get_page_of_virtual_address(*(get_regs(current_context)+REG_SP));
		printf("stack page %ld ",stack_page);
		printf("frame pointer 0x%lX ",*(get_regs(current_context)+REG_S0));
		printf("ra 0x%lX ",*(get_regs(current_context)+REG_RA));
		printf("gp 0x%lX \n",*(get_regs(current_context)+REG_GP));
		*/
    if (is_code_address(current_context, pc))
    {
      ...
      return;
			...
			printf("code address segmentation fault \n");
...
void print_profile()
{
  ...
  {
    printf("%s: ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n", selfie_name);
    printf("%s: context: %s %p \n", selfie_name, get_name(context),context);
...
void init_context(uint64_t *context, uint64_t *parent, uint64_t *vctxt)
{
  ...
  set_use_gc_kernel(context, GC_DISABLED);
	...
	//pids
	set_pid(context,pid_initializer++);
	set_children_list_head(context,(uint64_t*)0);
	set_n_children(context,(uint64_t)0);
	set_child_exit_code(context,(uint64_t)0);
	set_next_sibling(context,(uint64_t *)0);
	set_prev_sibling(context,(uint64_t *)0);
	set_state(context,READY);
...
void thread_context(uint64_t * context, uint64_t * new_context){
	...
	*(new_regs+REG_S0) = 0;
	...
	set_mc_mapped_heap(new_context,get_mc_mapped_heap(context));
	set_pt(new_context,get_pt(context));
	set_thread_id(new_context,get_n_threads(context)+1);
	set_n_threads(context,get_n_threads(context)+1);


	
	uint64_t begin_context_stack = HIGHESTVIRTUALADDRESS;
	uint64_t end_context_stack = *(get_regs(context)+REG_SP);
	
	if(get_next_thread(context) != (uint64_t*)0){
		begin_context_stack = get_lowest_hi_page(get_next_thread(context));
	...
	while(begin_context_stack != end_context_stack){
		map_and_store(new_context,begin_new_context_stack,load_virtual_memory(get_pt(context),begin_context_stack));
		begin_context_stack--;begin_new_context_stack--;
	}
	
	begin_context_stack = HIGHESTVIRTUALADDRESS;
	if(get_next_thread(context) != (uint64_t*)0){
		begin_context_stack = get_lowest_hi_page(get_next_thread(context));
	}

	//*(get_regs(new_context)+REG_SP) = end_context_stack - offset;
	*(get_regs(new_context)+REG_SP) = end_context_stack - offset - (begin_context_stack - end_context_stack);
	*(get_regs(new_context)+REG_S0) = *(get_regs(new_context)+REG_SP);

	uint64_t new_sp_top = *(get_regs(new_context)+REG_SP);
	uint64_t new_sp_bottom = end_context_stack - offset;
	set_lowest_hi_page(new_context,new_sp_bottom);
	set_highest_hi_page(new_context,new_sp_top);
	set_mc_stack_peak(new_context,new_sp_top);
	set_lc_malloc(new_context,get_lc_malloc(context));


	set_thread_id(new_context,get_thread_id(context)+1);
}
...
uint64_t handle_system_call(uint64_t *context)
{
  ...
  if (a7 == SYSCALL_BRK)
  {
	...
	else if (a7 == SYSCALL_THREAD)
		implement_thread(context);
	else if (a7 == SYSCALL_TS)
		implement_ts(context);
	else if (a7 == SYSCALL_CAS)
		implement_cas(context);
	...
	else if (a7 == SYSCALL_GET_CONTEXT)
		implement_get_context(context);
	else if (a7 == SYSCALL_SLEEP)
		implement_sleep(context);
	else if (a7 == SYSCALL_AWAKE)
  ...
  {	
		...
		if(parent != (uint64_t*)0){
			...
			if(get_n_children(parent) == 0 && get_state(parent) == BLOCKED){
				delete_from_list(parent,&blocked_contexts);
				set_next_context(parent,used_contexts);
				set_prev_context(parent,(uint64_t*)0);
				if(used_contexts != (uint64_t*)0)
				set_prev_context(used_contexts,parent);
				used_contexts = parent;
				set_child_exit_code(parent,EXITCODE_UNKNOWNSYSCALL);
				set_state(parent,READY);
...
uint64_t handle_exception(uint64_t *context)
{
  ...
  {
    printf("%s: context %p %s threw uncaught exception: ", selfie_name, context ,get_name(context));
...
uint64_t selfie_run(uint64_t machine)
{
  ...
  init_memory(atoi(peek_argument_val));

	for(uint64_t i=0;i<n_instances;i++){
		uint64_t * temp = create_context(MY_CONTEXT,0);
		boot_loader(temp);	
		print_context(temp);
		printf("%p init context \n",temp );
	}

	current_context = used_contexts;
//

	//*(test_context+18) = (uint64_t)current_context; 
    ...
    while (number_of_remaining_arguments() > 0)
    {
      ...
      else if (extras == 0)
      {
        if (string_compare(argument, "-m")){  
					peek_argument_val = peek_argument(0);
					if(number_of_remaining_arguments() > 1){
						get_argument();
						get_argument();
						if(string_compare(argument, "-x")){
							n_instances = atoi(peek_argument(0));
						}
					}
